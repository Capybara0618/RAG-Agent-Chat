...
- 存储：默认使用 PostgreSQL，驱动为 `psycopg`
- 检索：混合检索 + 确定性 embedding
- 前端：Streamlit

## 快速开始

先准备一个本地 PostgreSQL 数据库，并创建 `knowledgeops` 数据库。最简单的方式是直接用 Docker 启动：
...
测试环境请单独准备 PostgreSQL，并配置：

```env
TEST_DATABASE_URL=postgresql+psycopg://postgres:postgres@localhost:5432/knowledgeops_test
```
...
- 如果想把它进一步升级成更强的求职项目，可以继续接入 OpenAI 兼容模型接口、引入 pgvector，并连接真实企业知识源
